 ...
 *
 * @param lockToken   proof that this claim, and not a later one, holds the row.
 *                    Every transition is guarded by it, so a worker whose lock
 *                    was reclaimed while it was settling updates zero rows
 *                    instead of overwriting the state of the worker that holds
 *                    the row now. Regenerated by each claim, which is what makes
 *                    "the row is PROCESSING" and "I still hold the row" two
 *                    different questions
